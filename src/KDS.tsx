...
export const RestaurantKDS = () => {
  ...
  const todayRevenue = orders.filter(o => o.status !== 'cancelled').reduce((a, o) => a + (o.subtotal ?? o.total), 0);
          ...
          {advanced.map(order => (
            <div key={order.id} className="bg-gray-900 border-2 border-purple-500/50 rounded-2xl p-5">
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="text-right">
                  <p className="text-2xl font-bold text-purple-400">${sf(order.subtotal ?? order.total)}</p>
